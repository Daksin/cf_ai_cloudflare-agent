...
Do NOT call tools for greetings or simple conversation, respond normally without calling any tools.
`,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    // 1) Agent endpoints (WebSocket + HTTP)
    const routed = await routeAgentRequest(request, env);
    if (routed) return routed;

    // 2) Otherwise serve the React app (built into /dist)
